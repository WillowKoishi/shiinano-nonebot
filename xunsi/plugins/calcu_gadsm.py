...
calcu_gadsm = on_command("锘 GADSM",aliases={"锘 引力弹弓","锘GADSM","锘引力弹弓","锘yldg","锘 瞎几把飞","锘瞎几把飞"},priority=10,block=True)
...
async def gadsm_test_method(event:Event,arg:Message=CommandArg()):
    if (event.get_user_id())=="1033855007":
        ...
        await gadsm_test.finish(MessageSegment.at(event.get_user_id())+MessageSegment.text(f"您本次的计算参数和结果为:\n{resalt[0]}")+MessageSegment.image(f"file:///C:\\Users\\Administrator\\shiinano\\xunsi\\plugins\\GADSM\\GADSM\\{resalt[1]}"))
...
@calcu_gadsm.got("gadsm_arg", prompt='''请输入GADSM计算参数：
...
async def calcu_gadsm_function(event:Event,gadsm_arg: str = ArgPlainText()):
    ...
    await calcu_gadsm.finish(MessageSegment.at(event.get_user_id())+MessageSegment.text(f"您本次的计算参数和结果为:\n{result[0]}")
                             +MessageSegment.image(f"file:///C:\\Users\\Administrator\\shiinano\\xunsi\\plugins\\GADSM\\GADSM\\{result[1]}")
