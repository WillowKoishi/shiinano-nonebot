...
calcu_dv_trigger = on_command("锘 计算dv",aliases={"锘 jsdv","锘jsdv","锘计算dv","锘 计算DV","锘 JSDV","锘 jsdv"},priority=10,block=True)
...
@calcu_dv_trigger.got("dv_arg", prompt='''请输入dv计算参数
格式为  “A [比冲] [全重] [油重]”
或      “B [比冲] [全重] [干重]”
...
async def got_calcu_dv_function(dv_arg: str = ArgPlainText()):
    ...
    if float(dv_arg_list[1]) ==0:
        await calcu_dv_trigger.finish("你这发动机根本喷不出任何东西！\ntips:发动机比冲必须大于0")
    if float(dv_arg_list[1]) < 0:
        await calcu_dv_trigger.finish("你这发动机怎么会吸气的！\ntips:发动机比冲必须大于0")
    ...
    if (float(dv_arg_list[2]) <0)or(float(dv_arg_list[3]) <0):
        await calcu_dv_trigger.finish("如果负质量能稳定在宏观条件存在，还需要什么化学火箭！不干了！\ntips:所有参数不能为负")

    if (float(dv_arg_list[2]) <= float(dv_arg_list[3])):
        await calcu_dv_trigger.finish("燃料质量怎么可能比总质量还多呢！不干了！\ntips:油重或干重不能大于等于总质量")
    ...
    if dv_arg_list[0] in ["A","a"]:
        ...
        ackeret_dv_up   = 1-math.pow((m_full/(m_full-m_fule)),(2*isp*9.81/c_light))
        ackeret_dv_down = 1+math.pow((m_full/(m_full-m_fule)),(2*isp*9.81/c_light))
        ackeret_dv = -c_light * ackeret_dv_up/ ackeret_dv_down
        await calcu_dv_trigger.finish(f"计算结果：\n干质比：{round(m_full/(m_full-m_fule),3)}\nΔv={round(ackeret_dv,3)}m/s")#\n使用阿克莱公式：Δv={round(ackeret_dv,3)}m/s")
...
@calcu_dwr_trigger.got("dwr_arg", prompt='''请输入干质比计算参数
格式为  “[比冲] [目标Δv]”''')
async def got_calcu_dwr_function(dwr_arg: str = ArgPlainText()):
    ...
    if not(is_number(dwr_arg_list[0]) or is_number(dwr_arg_list[1])):
        await calcu_dwr_trigger.finish("你在乱输什么参数！不干了！\ntips:参数1或2错误，只能为数字")
...
calcu_hohm_trigger = on_command("锘 霍曼转移",aliases={"锘 hohmann","锘霍曼转移","锘hmzy"},priority=10,block=True)
...
@calcu_hohm_trigger.got("hohmann_arg",prompt='''请输入霍曼转移计算参数
...
@calcu_kyl_trigger.got("kyl_arg",prompt="请输入康永来公式所需参数：[发动机数量] [发动机推力]")
...
def comprehensive_sensor_calculations(altitude, aperture, f_number, pixel_size, off_nadir_angle, wavelength_dict, earth_radius=6378000):
    ...
    def calculate_gsd(altitude, pixel_size, focal_length, off_nadir_angle):
        ...
        return (altitude * pixel_size / focal_length) / np.cos(np.radians(off_nadir_angle))
    ...
    # 计算幅宽，包含地球曲率和侧摆角度
    def calculate_swath_with_curvature(altitude, field_of_view, off_nadir_angle=0, earth_radius=6378000):
        # 有效高度计算，包含地球曲率
        effective_altitude = earth_radius * (np.sqrt(1 + 2 * altitude / earth_radius) - 1)
        # 幅宽计算公式：2 * 有效高度 * tan((视场角 + 侧摆角) / 2)
        return 2 * effective_altitude * np.tan((field_of_view + np.radians(off_nadir_angle)) / 2)
    ...
    # 将所有数据组织成 DataFrame
    results = {
        'Parameter': ['GSD (m)', 'Potential Swath (km)', 'Near Contact Swath (km)'] + [f'GRD - {band} (m)' for band in wavelength_dict],
        'Value': [gsd, potential_swath / 1000, near_contact_swath / 1000] + list(grd_data.values())
    }
...
# 运行综合传感器计算
df_results = comprehensive_sensor_calculations(altitude, aperture, f_number, pixel_size, off_nadir_angle, wavelength_dict)
...
@calcu_gsd_trigger.got("gsd_arg",prompt='''===光学卫星简单计算器===
请输入计算所需的参数
